...
CI is **configured, not executed remotely**. The upstream fast-test workflow references organization-specific AWS runners and images that may not be available on a personal fork. The additive job uses hosted Ubuntu and the scoped CPU suite. This does not demonstrate all upstream jobs are runnable on that fork.
...
"$STAGING_DIR/consumer/bin/python" -m pip install -r "$CONTRIBUTION_DIR/.ramp-kit/runtime/requirements.lock"
