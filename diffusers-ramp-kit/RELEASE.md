...
The kit repository has a root `.github/workflows/test.yml` because this repository stores the kit in a subfolder. The bundled `.github/workflows/test.yml` supports a standalone kit checkout. Neither is evidence that a Diffusers fork PR has run.
