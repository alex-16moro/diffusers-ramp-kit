...
All commands run from the Diffusers checkout with the prepared Python environment active.
...
python .ramp-kit/run.py --repo . prepare --spec .ramp-kit/examples/empty-timesteps.json
```
...
The scaffold appears under `.ramp/empty-timesteps/test-scaffold.txt`. Fill its methods inside the existing `DDPMSchedulerTest` class. The task record maps acceptance criteria to those methods. Keep the test file unchanged between the baseline and candidate runs.
...
If tests change after baseline evidence, replay the same new tests against a disposable copy of the original implementation. Do not reset the user's checkout. Baseline test hash must match the candidate's test file.

## Human handoff

Open `.ramp/empty-timesteps/review.html` or `review.md`; inspect the patch and `PR-DRAFT.md`. The PM reviews criteria; QA reviews mapped tests and the before/after evidence; DevOps reviews CI wiring and remaining release gates. Publication and merge remain human decisions.
