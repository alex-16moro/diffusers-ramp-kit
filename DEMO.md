...
1. **Engineer request (1 minute).** Follow START_HERE.md on a clean engineer host without the kit repo or solutions. Open a fresh Cursor session on its clone of the reviewed fork `ramp-base` and ask in ordinary language for a clear error on empty custom timesteps, suggesting `[999]`. Show automatic discovery of `.cursor/rules/ramp-entry.mdc` and the task recipe. Cursor runtime integration must be observed live; CLI checks alone do not establish that it works.
